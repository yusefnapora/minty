...
pub fun fetch(address: Address, id: UInt64): AccountItem? {
  if let col = getAccount(address).getCapability<&CoolCats.Collection{NonFungibleToken.CollectionPublic, CoolCats.CoolCatsCollectionPublic}>(CoolCats.CollectionPublicPath).borrow() {
    if let item = col.borrowCoolCats(id: id) {
