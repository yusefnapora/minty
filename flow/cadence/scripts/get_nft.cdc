...
pub fun fetch(address: Address, id: UInt64): AccountItem? {
  if let col = getAccount(address).getCapability<&NextToken.Collection{NonFungibleToken.CollectionPublic, NextToken.NextTokenCollectionPublic}>(NextToken.CollectionPublicPath).borrow() {
    if let item = col.borrowNextToken(id: id) {
